...
/** Tempo de permanência exigido por marco. Ponto de partida pedido em 15/09. */
export const TOTEM_CHARGE_SECONDS=[45,55,65,75] as const;
...
export interface TotemSite {id:string;name:string;index:number;position:Vec3;radius:number;chargeSeconds:number}
...
export function planExpedition(world:ExpeditionTerrain,anchors:readonly TotemAnchor[],origin:Vec3,reachable:(p:Vec3)=>boolean,count=4,radius=TOTEM_RADIUS):TotemSite[] {
  ...
  for(const anchor of sorted){
    ...
    sites.push({id:anchor.id,name:anchor.name,index:sites.length,position,radius,chargeSeconds:TOTEM_CHARGE_SECONDS[sites.length]??TOTEM_CHARGE_SECONDS[TOTEM_CHARGE_SECONDS.length-1]!});
  }
  return sites;
}

/**
 * Objetivo padrão da expedição: quatro totens carregados por permanência, depois um chefe e a fenda.
...
export class ExpeditionObjectives {
  ...
  /** Carga por segundo dentro da área; a Ressonância da Colheita multiplica modestamente. */
  chargeMultiplier=1;
  ...
  activate(player:Vec3):TotemProgress|undefined {
    ...
    this.say(`MARCO ${candidate.site.index+1} ATIVO · ${candidate.site.name} · permaneça na área`,4);
    return candidate;
  }
  update(dt:number,player:Vec3,alive:boolean):void {
    ...
    if(totem&&totem.state!=='complete'){
      if(alive&&this.inside(totem,player)){
        ...
        totem.charged=Math.min(totem.site.chargeSeconds,totem.charged+dt*Math.max(0,this.chargeMultiplier));
        if(totem.charged>=totem.site.chargeSeconds){
          totem.state='complete';this.activeIndex=-1;this.rewardsPending++;
          this.say(`MARCO ${totem.site.index+1} CONCLUÍDO · recolha a recompensa`,6);
        }
  ...
  takeReward():boolean {if(this.rewardsPending<=0)return false;this.rewardsPending--;return true;}
