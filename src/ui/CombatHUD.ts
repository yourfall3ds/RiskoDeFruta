...
export class RunHUD {
  ...
  */
 /**
  * `interact` ausente é mapa SEM BAÚS — o Test Map V1.0. O painel da economia de baús some e o
  * resto do HUD de corrida (relógio, horda, XP, barras) segue igual.
  */
 update(run:RunProgression,swarm:EnemySwarm,interact:RunInteractables|undefined,camera:Camera,expedition?:{objectives:ExpeditionObjectives;resonance:HarvestResonance;mp:MPCharge;player:{x:number;y:number;z:number};weather?:WeatherCycle;journey?:StageJourneyView},economy?:EconomyMirror):void {
  /**
  ...
  const entry=interact?.nearest,loot=interact?.nearestLoot,atRift=Boolean(interact?.atRift);
  ...
  shown(this.interactBox,Boolean(entry||loot||totem||(riftOpen&&atRift)));
  this.interact.set(riftOpen&&atRift?'<b>[E] ATRAVESSAR A FENDA</b><span>Créditos restantes viram XP.</span>'
