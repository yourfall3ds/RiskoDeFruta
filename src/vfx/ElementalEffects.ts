...
interface Grain{mesh:AbstractMesh;velocity:Vector3;age:number;active:boolean;water:boolean}
...
export class ElementalEffects {
 ...
 emit(kind:ElementKind,origin:Vector3,scale=1):void{
  ...
  if(kind==='earth'||kind==='water'||kind==='explosion'){
   ...
   grains.forEach((g,i)=>{const a=i*2.399+s.serial,r=.4+(i%4)*.27;g.active=true;g.age=0;g.mesh.setEnabled(true);g.mesh.position.copyFrom(origin);g.mesh.scaling.set(s.scale*(.7+i%3*.3),s.scale*(g.water?1.7:1),s.scale);g.velocity.set(Math.sin(a)*r*s.scale,1.6+(i%4)*.5,Math.cos(a)*r*s.scale);});
  }
  this.update(0);
 }
 update(dt:number):void{
  ...
  for(const g of this.grains){if(!g.active)continue;g.age+=dt;g.velocity.y-=9.8*dt;g.mesh.position.addInPlace(g.velocity.scale(dt));g.mesh.rotation.x+=dt*3;g.mesh.rotation.z+=dt*2;const ground=this.collision?.groundAt(g.mesh.position.x,g.mesh.position.z,g.mesh.position.y+.2)??0;if(g.age>1.7||g.mesh.position.y<ground+.02){g.active=false;g.mesh.setEnabled(false);}}
 }
