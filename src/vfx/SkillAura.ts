...
export class SkillAura {
 ...
 async load(loader=()=>LoadAssetContainerAsync('/models/arcane-skill-ritual.glb',this.scene)):Promise<void>{
  try{const container=await loader();if(this.disposed){container.dispose();return;}this.container=container;container.addAllToScene();
   ...
   for(const name of ['GroundSigilRoot','RightAuraRoot','LeftAuraRoot']){
    ...
    if(name==='GroundSigilRoot')this.ground=root;else{this.guns.push(root);this.rotations.push(root.rotationQuaternion?.clone()??Quaternion.Identity());}
   }
 ...
 update(t:SkillTimeline,position:Vector3,weapons:DualPistols):void{
  ...
  if(active&&(!this.wasActive||t.elapsed<this.previousElapsed)){
   this.origin.copyFrom(position);const y=this.collision?.groundAt(position.x,position.z,position.y+.4);
   if(y!==undefined&&Number.isFinite(y)&&y>position.y-6)this.origin.y=y;
  }
  ...
  this.ground!.position.copyFrom(this.origin).addInPlaceFromFloats(0,.035,0);
  ...
  this.lights[2]!.position.copyFrom(this.origin).addInPlaceFromFloats(0,.45,0);this.lights[2]!.intensity=fade*1.4;
