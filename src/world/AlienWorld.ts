...
/**
 * Um disco voador do cenário, exposto para o jogo poder atirar nele.
 *
 * `mesh` é a maior malha do disco, que é o que o hitscan das pistolas testa. `commanded` desliga a
 * órbita decorativa: quando a investida (`SaucerRaid`) assume a nave, é ela quem escreve a posição.
 */
export interface SceneSaucer {
...
export class AlienWorld {
 ...
 /** Discos voadores do cenário, na ordem em que foram criados. */
 readonly saucers:SceneSaucer[]=[];
 ...
 async load():Promise<void>{
  ...
  for(const [i,p] of [[-30,26,48],[44,31,61]].entries()){
   const root=instance('FruitSaucer');root.scaling.setAll(1.5);
   this.decoration.push({root,x:p[0]!,y:p[1]!,z:p[2]!,phase:i*3,saucer:true});
   // O disco deixa de ser cenário intocável: vira alvo do hitscan. Sem `isPickable` o raio das
   // pistolas atravessa a nave e a represália nunca poderia ser disparada por um tiro.
   const meshes=root.getChildMeshes().filter(m=>m.getTotalVertices()>0) as Mesh[];
   const body=[...meshes].sort((a,b)=>b.getTotalVertices()-a.getTotalVertices())[0];
   if(!body)continue;
   for(const mesh of meshes)mesh.isPickable=true;
   this.saucers.push({index:i,root,mesh:body,meshes,commanded:false});
  }
  this.update(0);
 }
 ...
 /** Posição que a órbita decorativa daria a um disco neste instante. É de onde a investida parte. */
 saucerOrbit(index:number):{x:number;y:number;z:number}|undefined {
  const entry=this.decoration.filter(d=>d.saucer)[index];
  if(!entry)return undefined;
  const t=this.time*.14+entry.phase;
  return {x:entry.x+Math.sin(t)*9,y:entry.y+Math.sin(t*.7)*1.8,z:entry.z+Math.cos(t)*5};
 }
 update(dt:number):void {
  this.time+=dt;
  let saucerIndex=0;
  for(const d of this.decoration){
   const t=this.time*(d.saucer?.14:.09)+d.phase;
   // Disco sob comando da investida: quem escreve a posição é o `SaucerRaid`, não a órbita.
   // Sem esta cessão a nave hostil voltaria para a órbita a cada quadro e nunca chegaria no jogador.
   const commanded=d.saucer&&(this.saucers[saucerIndex]?.commanded??false);
   if(d.saucer)saucerIndex++;
   if(commanded){d.root.rotation.set(0,t*1.9,0);continue;}
   d.root.position.set(d.x+Math.sin(t)*(d.saucer?9:1.8),d.y+Math.sin(t*.7)*(d.saucer?1.8:.65),d.z+Math.cos(t)*(d.saucer?5:1.2));
   d.root.rotation.set(d.saucer?Math.sin(t)*.08:0,t*(d.saucer?.6:.07),d.saucer?Math.cos(t)*.10:0);
  }
 }
