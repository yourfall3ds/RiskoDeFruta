...
interface Face {a:Vec3;b:Vec3;c:Vec3;ny:number;den:number}
...
export class TriangleGround {
  ...
  constructor(positions:readonly number[],indices:readonly number[]){
    ...
    for(let i=0;i<indices.length;i+=3){const a=point(indices[i]!),b=point(indices[i+1]!),c=point(indices[i+2]!),ux=b.x-a.x,uy=b.y-a.y,uz=b.z-a.z,vx=c.x-a.x,vy=c.y-a.y,vz=c.z-a.z;
      const nx=uy*vz-uz*vy,ny=uz*vx-ux*vz,nz=ux*vy-uy*vx,length=Math.hypot(nx,ny,nz);if(length<1e-8||Math.abs(ny)/length<.25)continue;
      const den=(b.z-c.z)*(a.x-c.x)+(c.x-b.x)*(a.z-c.z);if(Math.abs(den)<1e-7)continue;const face={a,b,c,ny:Math.abs(ny)/length,den};
