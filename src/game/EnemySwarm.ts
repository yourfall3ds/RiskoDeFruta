...
import { ENEMIES,MonsterDirector,type EnemyKind } from '../run/MonsterDirector';
...
export class EnemySwarm {
  ...
  constructor(private readonly scene:Scene,private readonly world:{targets:TrainingTarget[];collision:CollisionWorld},private readonly events:EventBus<GameEvents>,private readonly shadows:ShadowGenerator,private readonly player:PlayerMotor,private readonly progression:RunProgression,private readonly rng:RunRNG,readonly hordeMode=false){this.debris=new CorpseDebris(world.collision);this.procs=new ItemProcs(progression,rng.stream('procs'));this.director=new MonsterDirector(rng.stream('director'),progression.stage,50,hordeMode);this.effects=new CombatPresentation(scene);this.lasers=new EnemyLaser(scene);this.elemental=new ElementalEffects(scene,world.collision);for(let i=0;i<2;i++){const light=new PointLight('tomato-incendiary-charge-'+i,Vector3.Zero(),scene);light.diffuse=new Color3(1,.23,.025);light.range=4;light.intensity=0;this.chargeLights.push(light);}}
  ...
  nextStage():void {this.lasers.begin();this.elemental.clear();for(const l of this.chargeLights)l.intensity=0;this.burning.clear();this.debris.clear();this.ragdolls.clear();this.tactical?.clear();this.scheduler.clear();this.populationCap=this.budget.limit;this.benchmark=false;this.retirementClock=0;for(const a of this.actors){a.active=false;a.root.setEnabled(false);a.body.isPickable=false;}this.kills=0;this.boss=undefined;this.bossDeadTime=-1;this.effects.clear();this.labels.length=0;this.director=new MonsterDirector(this.rng.stream('director'),this.progression.stage,50,this.hordeMode);}
  get count():number{return this.actors.filter(x=>x.active&&!x.health.dead).length;}
  ...
  updateBudget(dt:number,frameMs:number):void {
    ...
    const actor=this.actors.filter(a=>a.active&&!a.health.dead&&a.kind!=='boss'&&distance(a.root.position,this.player.position)>18).sort((a,b)=>distance(b.root.position,this.player.position)-distance(a.root.position,this.player.position))[0];
  ...
  private spawnPosition():Vec3|undefined {return chooseSpawnAround(this.player.position,this.rng.stream('spawn'),this.world.collision,p=>Boolean(this.tactical?this.tactical.reachable(p,this.player.position):this.navigation?.reachable(p)),p=>this.actors.some(a=>a.active&&!a.health.dead&&distance(p,a.root.position)<2));}
  spawn(kind:EnemyKind,position?:Vec3,variant:EnemyVariant=kind==='boss'?'normal':chooseVariant(this.rng.stream('elite').next(),this.director.time)):boolean {
    ...
    if(this.count>=this.populationCap){const retired=this.actors.filter(a=>a.active&&!a.health.dead&&a.kind!=='boss').sort((a,b)=>distance(b.root.position,this.player.position)-distance(a.root.position,this.player.position))[0];if(!retired)return false;this.director.retireLivingEnemy();retired.active=false;this.scheduler.remove(retired.id);this.tactical?.remove(retired.id);retired.root.setEnabled(false);for(const mesh of retired.target.meshes??[retired.body])mesh.isPickable=false;}
  ...
  private hit(a:Actor,context:DamageContext):void {
    ...
    if(a.health.dead){this.scheduler.remove(a.id);a.state='dead';a.time=0;this.tactical?.remove(a.id);a.palette.sync();a.ragdoll=this.ragdolls.create(a.skeleton,a.body,applied,a.scale);this.audio?.enemy('death',a.kind,distance(a.root.position,this.player.position));a.body.isPickable=false;a.deathVelocity.set(context.forceDirection.x*3,3,context.forceDirection.z*3);this.kills++;this.progression.reward(a.kind==='boss',ENEMY_AFFIXES[a.variant].gold);const heal=this.procs.onKill();this.player.hp=Math.min(this.player.maxHP,this.player.hp+heal);this.effects.burst(a.root.position,heal?'energy':'juice',a.kind==='boss'?4:1.2);
      for(const mesh of a.target.meshes??[a.body])mesh.isPickable=false;
      if(a.kind==='boss'){this.debris.fracture(a.target.meshes??[a.body],a.body);for(const corpse of this.actors)if(corpse!==a&&corpse.active&&corpse.health.dead&&distance(corpse.root.position,a.root.position)<12){corpse.deathVelocity.x+=(corpse.root.position.x-a.root.position.x)*.8;corpse.deathVelocity.z+=(corpse.root.position.z-a.root.position.z)*.8;corpse.deathVelocity.y=5;}this.director.bossKilled();this.bossDeadTime=this.hordeMode?-1:0;this.events.emit('BossKilled',applied);this.events.emit('StageCompleted',{stageId:String(this.progression.stage)});}
    }
  }
  private think(a:Actor,dt:number):void {
    ...
    if(this.tactical){const group=this.actors.filter(other=>other.active&&!other.health.dead&&Boolean(ENEMY_BEHAVIORS[other.kind].ranged)===Boolean(behavior.ranged)).sort((x,y)=>distance(x.root.position,p)-distance(y.root.position,p));const rank=group.indexOf(a);this.tactical.target(a.id,p,rank,Boolean(behavior.ranged),def.speed*ENEMY_AFFIXES[a.variant].speed);}
    ...
    a.cooldown=Math.max(0,a.cooldown-dt);const attacking=this.actors.filter(other=>other.active&&other.state==='windup'&&Boolean(ENEMY_BEHAVIORS[other.kind].ranged)===Boolean(behavior.ranged)).length;const blocked=this.world.collision.sweepSphere({x:a.root.position.x,y:a.root.position.y+1.1,z:a.root.position.z},{x:p.x-a.root.position.x,y:p.y-a.root.position.y,z:p.z-a.root.position.z},.05);if(d<=def.range&&Math.abs(p.y-a.root.position.y)<3&&a.cooldown===0&&attacking<(behavior.ranged?4:3)&&!blocked){a.state='windup';a.time=0;this.tactical?.velocity(a.id,{x:0,y:0,z:0},0);this.audio?.enemy('windup',a.kind,d);a.locked={...p};a.attack++;a.direction={x:0,z:0};this.effects.warning({...p,y:this.world.collision.groundAt(p.x,p.z,p.y+.5)},behavior.warningRadius,behavior.windup,0,a.id,'intent');if(behavior.warningShape==='cone'&&a.attack%5===4)this.effects.cone(a.root.position,a.locked,7,behavior.windup,a.id);if(behavior.warningShape==='line')this.effects.line(a.root.position,a.locked,1.2,behavior.windup,a.id);}
  }
  ...
  fixedUpdate(dt:number):void {
    ...
    for(const a of this.actors){if(!a.active)continue;a.time+=dt;a.hit=Math.max(0,a.hit-dt);a.stagger=Math.max(0,a.stagger-dt);a.staggerCooldown=Math.max(0,a.staggerCooldown-dt);
      ...
      const behavior=ENEMY_BEHAVIORS[a.kind];
      if(this.tactical){const p=this.tactical.position(a.id);if(p){const ground=this.world.collision.groundAt(p.x,p.z,p.y+.55);a.root.position.set(p.x,Number.isFinite(ground)?ground:p.y,p.z);}}
    ...
    if(!this.tactical){
    // Local separation only; avoid an all-pairs pass for a larger population.
    const buckets=new Map<string,Actor[]>();for(const a of this.actors)if(a.active&&!a.health.dead){const key=`${Math.floor(a.root.position.x/3)},${Math.floor(a.root.position.z/3)}`;const list=buckets.get(key)??[];list.push(a);buckets.set(key,list);}
    for(const a of this.actors)if(a.active&&a.state==='chase'){const x=Math.floor(a.root.position.x/3),z=Math.floor(a.root.position.z/3);let remainingPairs=8;neighbors:for(let dx=-1;dx<=1;dx++)for(let dz=-1;dz<=1;dz++)for(const b of buckets.get(`${x+dx},${z+dz}`)??[]){if(a.id>=b.id)continue;if(--remainingPairs<0)break neighbors;const d=distance(a.root.position,b.root.position),min=ENEMIES[a.kind].radius+ENEMIES[b.kind].radius;if(d>.01&&d<min){const push=(min-d)*dt*2,px=(a.root.position.x-b.root.position.x)/d*push,pz=(a.root.position.z-b.root.position.z)/d*push;this.world.collision.move(a.root.position,px,pz,.4,1.8,.8);this.world.collision.move(b.root.position,-px,-pz,.4,1.8,.8);}}}
    }
  ...
  update(dt:number):void {
    ...
    const charging=this.actors.filter(a=>a.active&&!a.health.dead&&a.kind==='tomato'&&a.state==='windup').sort((a,b)=>distance(a.root.position,this.player.position)-distance(b.root.position,this.player.position));
    for(let i=0;i<this.chargeLights.length;i++){const light=this.chargeLights[i]!,actor=charging[i];light.intensity=actor?Math.pow(Math.min(1,actor.time/ENEMY_BEHAVIORS.tomato.windup),2)*3:0;if(actor){light.position.copyFrom(actor.root.position).addInPlaceFromFloats(0,3,0);light.includedOnlyMeshes=[...(actor.target.meshes??[actor.body])];}}
    this.debris.update(dt);this.effects.render(dt);this.shadowClock-=dt;
    if(this.shadowClock<=0){this.shadowClock=.5;for(const mesh of this.shadowCasters)this.shadows.removeShadowCaster(mesh);this.shadowCasters=this.actors.filter(a=>a.active&&!a.health.dead&&distance(a.root.position,this.player.position)<24).sort((a,b)=>distance(a.root.position,this.player.position)-distance(b.root.position,this.player.position)).slice(0,4).map(a=>a.body);for(const mesh of this.shadowCasters)this.shadows.addShadowCaster(mesh);}
    ...
    for(const a of this.actors){
      ...
      for(const mesh of a.target.meshes??[a.body]){const charge=a.kind==='tomato'&&a.state==='windup'&&!a.health.dead?Math.min(1,a.time/ENEMY_BEHAVIORS.tomato.windup):0;mesh.renderOverlay=(a.hit>0||charge>0)&&!a.health.dead;mesh.overlayColor=charge>0?new Color3(1,.25,.025):Color3.FromHexString('#fff0bc');mesh.overlayAlpha=charge>0?.65*charge:Math.min(.48,a.hit*5);}
      ...
      if(a.kind==='carrot'&&a.laserArm&&a.laserSocket&&(a.state==='windup'||a.state==='recover'&&a.time<.4)){const direction=new Vector3(a.locked.x,a.locked.y+.9,a.locked.z).subtract(a.laserSocket.getAbsolutePosition()).normalize();aimArmAt(a.laserArm,a.laserSocket,direction);const path=this.laserPath(a);this.lasers.show(path.from,path.to,a.state==='windup'?.09+.10*a.time:1-a.time/.4);}
      a.palette.sync();a.root.computeWorldMatrix(true);
    }
  }
  dispose():void {this.lasers.dispose();for(const l of this.chargeLights)l.dispose();this.elemental.dispose();this.burning.clear();this.ragdolls.clear();this.tactical?.dispose();this.debris.clear();this.disposed=true;this.scheduler.clear();this.effects.clear();for(const a of this.actors){this.world.collision.playerBodies.delete(a.id);const i=this.world.targets.indexOf(a.target);if(i>=0)this.world.targets.splice(i,1);for(const clip of a.clips.values())clip.dispose();a.root.dispose();}for(const container of this.containers.values())container.dispose();this.actors.length=0;}
}
