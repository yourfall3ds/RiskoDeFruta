...
import {BARN_CHESTS} from '../src/world/ExplorationSites';
...
it.each([0,1,2,3,4])('walks across authored city bridge section %i without falling or clipping its landing',index=>{
 ...
 for(let i=0;i<Math.ceil((distance+.4)/WALK_SPEED*60)+30;i++){if(Math.hypot(player.position.x-b.x,player.position.z-b.z)<.25)break;player.fixedUpdate(1/60,{...EMPTY_INPUT,z:1},yaw);}
...
it('the baked routes reach their destinations rather than accepting partial paths',()=>{const routes=JSON.parse(readFileSync('docs/navmesh-validation.json','utf8'));expect(routes).toHaveLength(52);expect(routes.map((r:{name:string})=>r.name)).toEqual(expect.arrayContaining(['barn','west bridge','east bridge','city access','northern farm','market','orchard frontier','grain port','glasshouse district','west greenhouse aisle','east greenhouse aisle','loot access root-grove-entry','loot access root-grove-square','loot access root-grove-east','loot access root-mill-entry','loot access root-mill-square','loot access root-mill-south','loot access root-seed-entry','loot access root-seed-square','loot access root-seed-south']));expect(routes.every((r:{success:boolean})=>r.success)).toBe(true);});
// Uma rota por baú de celeiro: o interior só conta como jogável se o caminho assado entra pela porta
// e chega ao baú, e não apenas se o baú tem chão embaixo.
it('every barn chest has a baked route from the arrival field into the room',()=>{const routes=JSON.parse(readFileSync('docs/navmesh-validation.json','utf8')) as {name:string;success:boolean;path:unknown[]}[];const barn=routes.filter(r=>r.name.startsWith('barn loot '));expect(barn).toHaveLength(BARN_CHESTS.length);for(const route of barn){expect(route.success,route.name).toBe(true);expect(route.path.length,route.name).toBeGreaterThan(1);}});
