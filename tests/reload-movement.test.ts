...
it('keeps sprint, jump and dodge responsive while a reload suppresses firing intent',()=>{
 ...
 expect(p.sprinting).toBe(true);expect(p.velocity.z).toBeCloseTo(SPRINT_SPEED,1);
...
it('layers reload arms over moving legs without importing the reload root translation',async()=>{
 ...
 const p=new PlayerMotor(new CollisionWorld(),new EventBus(),{x:0,y:0,z:0});p.velocity.z=SPRINT_SPEED;p.sprinting=true;visual.ready=true;visual.reloadProgress=.5;
