...
it('plays a recording for every player shot even immediately after corn fires',async()=>{
 ...
 vi.stubGlobal('AudioContext',Context);vi.stubGlobal('fetch',vi.fn(async()=>({ok:true,json:async()=>({pistol:['/pistol.wav'],throw:['/corn-throw.wav']}),arrayBuffer:async()=>new ArrayBuffer(8)})));
 const audio=new WeaponAudio();try{audio.unlock();await vi.waitFor(()=>expect(decoded).toBe(5));audio.enemy('attack','corn',2);audio.shot(false);audio.shot(false);expect(starts).toEqual([2,2,2]);}finally{audio.dispose();vi.unstubAllGlobals();}
});
